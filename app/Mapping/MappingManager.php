...
class MappingManager {
	...
	public function getMapping( string $schema_type, $id = null ): array {
		...
		return $this->mappings[ $schema_type ]->getMapping( $schema_type, $id );
	}
	 ...
	 *
	 */
	public function saveMapping( string $schema_type, array $mappingData, $id = null ): void {
		if ( ! isset( $this->mappings[ $schema_type ] ) ) {
			throw new \RuntimeException( 'No mapping registered for schema type: ' . $schema_type );
		}

		$this->mappings[ $schema_type ]->saveMapping( $schema_type, $mappingData, $id );
	}
	 ...
	 */
	public function resetMappingToDefault( string $schema_type, $id = null ): void {
		if ( ! isset( $this->mappings[ $schema_type ] ) ) {
			throw new \RuntimeException( 'No mapping registered for schema type: ' . $schema_type );
		}
		...
		$this->mappings[ $schema_type ]->saveMapping( $schema_type, $defaultMapping, $id );
	}
