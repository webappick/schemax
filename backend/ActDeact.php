...
class ActDeact extends Base {
	...
	public static function upgrade_procedure() {
		...
		$version = \strval( \get_option( 'schemax-version' ) );

		if ( !\version_compare( SMAX_VERSION, $version, '>' ) ) {
			return;
		}

		\update_option( 'schemax-version', SMAX_VERSION );
		\delete_option( SMAX_TEXTDOMAIN . '_fake-meta' );
	}
