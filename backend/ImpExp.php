...
class ImpExp extends Base {
	...
	public function settings_export() {
		...
		$settings      = array();
		$settings[ 0 ] = \get_option( SMAX_TEXTDOMAIN . '-settings' );
		$settings[ 1 ] = \get_option( SMAX_TEXTDOMAIN . '-settings-second' );

		\ignore_user_abort( true );

		\nocache_headers();
		\header( 'Content-Type: application/json; charset=utf-8' );
		\header( 'Content-Disposition: attachment; filename=schemax-settings-export-' . \gmdate( 'm-d-Y' ) . '.json' );
		\header( 'Expires: 0' );
	...
	public function settings_import() {
		...
		if ( 'json' !== $extension ) {
			\wp_die( \esc_html__( 'Please upload a valid .json file', SMAX_TEXTDOMAIN ) );
		}

		$import_file = $_FILES[ 's_import_file' ][ 'tmp_name' ]; //phpcs:ignore WordPress.Security.ValidatedSanitizedInput

		if ( empty( $import_file ) ) {
			\wp_die( \esc_html__( 'Please upload a file to import', SMAX_TEXTDOMAIN ) );
		}
		...
		if ( !$settings_file ) {
			...
			if ( \is_array( $settings ) ) {
				\update_option( SMAX_TEXTDOMAIN . '-settings', \get_object_vars( $settings[ 0 ] ) );
				\update_option( SMAX_TEXTDOMAIN . '-settings-second', \get_object_vars( $settings[ 1 ] ) );
			}

			\wp_safe_redirect( \admin_url( 'options-general.php?page=' . SMAX_TEXTDOMAIN ) );
			exit;
		}

		new \WP_Error(
				'schemax_import_settings_failed',
				\__( 'Failed to import the settings.', SMAX_TEXTDOMAIN )
			);
	}
